...
resource "google_cloudbuild_trigger" "validate_prod" {
  ...
  filename = "demo/cicd/configs/tf-validate.yaml"

  substitutions = {
    _TERRAFORM_ROOT = "demo"
    _MANAGED_DIRS   = "groups audit org_policies folders"
    _WORKER_POOL    = ""
...
resource "google_cloudbuild_trigger" "plan_prod" {
  ...
  filename = "demo/cicd/configs/tf-plan.yaml"
...
resource "google_cloudbuild_trigger" "apply_prod" {
  ...
  filename = "demo/cicd/configs/tf-apply.yaml"
